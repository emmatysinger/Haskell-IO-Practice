...
fizzBuzz n f b = [ what x f b| x <- [1..n]] -- put your code here
what n f b = if n `mod` f == 0 && n `mod` b == 0 then "FizzBuzz"
              else if n `mod` f == 0 then "Fizz"
                    else if n `mod` b == 0 then "Buzz"
                          else show n
...
main = do
    ...
    numBuzz <- readLn
    mapM_ putStrLn (fizzBuzz numVar numFizz numBuzz) 
